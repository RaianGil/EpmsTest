...
namespace EPolicy.Customer
{
	public class Customer
	{
        ...
        private bool _SameAsMail = false;
        ...
        public bool SamesAsMail
        {
            get
            {
                return this._SameAsMail;
            }
            set
            {
                this._SameAsMail = value;
            }
        }
        ...
        public static DataTable DeleteDocumentsByDocumentsID(int DocID)
        {
            try
            {
        ...
        public static DataTable GetDocumentsByCustomerNo(string CustomerNo, int TaskContolID, int OldTaskContolID)
        {
            try
            {
        ...
        public static DataTable GetAgentByUserID(string UserID)
        {
        ...
        public static DataTable GetAgentByCustomer(string CustomerNo, string AgentID)
        {
            DbRequestXmlCookRequestItem[] cookItems =
            ...
            try
            {
                dt = exec.GetQuery("GetAgentByCustomer", xmlDoc);
                return dt;
            }
            catch (Exception ex)
            {
		...
		{
            if (!ProspectExistsForCurrentCustomer(Cust) && Cust.CustomerNo !="")
			{
				if(UserID == 0)
				...
				prospect.BusinessName =  Cust.BusinessName1;
		...
		public static DataTable GetCustomerByCriteria(string CustomerNo,string FirstName,string LastName1,string LastName2,string SocialSecurity,string Phone, bool IsBusiness,bool All, string EmployerSocialSecurity ,string BusinessName, int userId2, string LicenseNumber, string DOB)
		{
			DataTable dt = GetCustomerByCriteriaDB(CustomerNo,FirstName,LastName1,LastName2,SocialSecurity,Phone,IsBusiness,All,EmployerSocialSecurity,BusinessName, userId2, LicenseNumber, DOB);
        ...
        public static DataTable GetCustomerByUserID(int UserID)
        {
            try
            {
                ...
                try
                {
                    dt = exec.GetQuery("GetCustomerByUserID", xmlDoc);
                    return dt;
                }
                catch (Exception ex)
                {
        ...
        public static DataTable GetCustomerExists(string FirstName, string LastName, string BirthDate, string Email)
        {
            try
            {
                DbRequestXmlCookRequestItem[] cookItems = new DbRequestXmlCookRequestItem[3];
                DbRequestXmlCooker.AttachCookItem("FirstName", SqlDbType.VarChar, 100, FirstName.ToString(), ref cookItems);
                DbRequestXmlCooker.AttachCookItem("LastName", SqlDbType.VarChar, 100, LastName.ToString(), ref cookItems);
                DbRequestXmlCooker.AttachCookItem("BirthDate", SqlDbType.SmallDateTime, 0, BirthDate.ToString(), ref cookItems);
                DbRequestXmlCooker.AttachCookItem("Email", SqlDbType.Char, 50, Email.ToString(), ref cookItems);
        ...
        public static void AddComments(string CustomerNo, string Comment)
        {
            try
            {
                Baldrich.DBRequest.DBRequest Executor = new Baldrich.DBRequest.DBRequest();
                ...
                try
                {
                    ...
                    DbRequestXmlCooker.AttachCookItem("CustomerNo", SqlDbType.VarChar, 20, CustomerNo, ref cookItems);
                    DbRequestXmlCooker.AttachCookItem("Comment", SqlDbType.VarChar, 500, Comment, ref cookItems);
                    XmlDocument xmlDoc = DbRequestXmlCooker.Cook(cookItems);

                    Executor.BeginTrans();
                    id = Executor.Insert("AddComments", xmlDoc);
                    Executor.CommitTrans();
                }
                catch (Exception xcp)
                {
                    Executor.RollBackTrans();
                    throw new Exception("Error Could not Save the Comments, Try again.");
                }
            }
            catch (Exception exp)
            {
        ...
        public static void AddCustomerCommentsByTaskControlID(string TaskControlID, string Comment)
        {
            try
            {
                Baldrich.DBRequest.DBRequest Executor = new Baldrich.DBRequest.DBRequest();
                ...
                try
                {
                    ...
                    Executor.BeginTrans();
                    id = Executor.Insert("AddCustomerCommentsByTaskControlID", xmlDoc);
                    Executor.CommitTrans();
                }
                catch (Exception xcp)
                {
        ...
        public static void AddCustomerCommentsWithDateByTaskControlID(string TaskControlID, string Comment,string date)
        {
            try
            {
                Baldrich.DBRequest.DBRequest Executor = new Baldrich.DBRequest.DBRequest();
                ...
                try
                {
                    ...
                    DbRequestXmlCooker.AttachCookItem("TaskControlID", SqlDbType.Int, 0, TaskControlID, ref cookItems);
                    DbRequestXmlCooker.AttachCookItem("Comment", SqlDbType.VarChar, 500, Comment, ref cookItems);
                    DbRequestXmlCooker.AttachCookItem("Date", SqlDbType.DateTime, 0, date, ref cookItems);
                    XmlDocument xmlDoc = DbRequestXmlCooker.Cook(cookItems);

                    Executor.BeginTrans();
                    id = Executor.Insert("AddCustomerCommentsWithDateByTaskControlID", xmlDoc);
                    Executor.CommitTrans();
                }
                catch (Exception xcp)
                {
		...
		private void Validate()
		{
			...
			else					// Individual Customer
			{
                ...
                //    found = true;
                //}
//				else if (this.LastName2 == "" &&  found == false)
//				{
//					errorMessage = "LastName2 is missing or wrong.";
//					found = true;
//				}
...
//				}
        ...
        private void SaveCustomer(int UserID)
        {
            ...
            try
            {
                if (this.Birthday == "")
                {
                    this.Birthday = "01/01/1900";//DateTime.Now.AddYears(-30).ToShortDateString();
                }
               this.FileNo = DateTime.Parse(this.Birthday).Month.ToString().PadLeft(2, '0') +  DateTime.Parse(this.Birthday).Day.ToString().PadLeft(2, '0') + DateTime.Parse(this.Birthday).Year.ToString() + this.SocialSecurity;
               //Busca ese cliente si existe para buscar y no anadir el mismo sistema. (GetCustomerByFileNo)
               DataTable dtcust= GetCustomerByFileNo(this.FileNo);

               //if (dtcust.Rows.Count > 0)
               if (1 == 2)
               {
                   string custno = dtcust.Rows[0]["CustomerNo"].ToString();

                   EPolicy.Customer.Customer customer = Customer.GetCustomer(custno, UserID);
                   this.Mode = 2;
                   this.CustomerNo = customer.CustomerNo;

                   if (this.ProspectID == 0)
                        this.ProspectID = customer.ProspectID;
                   ...
                   if (this.SamesAsMail == null)
                       this.SamesAsMail = customer.SamesAsMail;

                   if (this.AddressPhysical1.Trim() == "")
                       this.AddressPhysical1 = customer.AddressPhysical1;

                   if (this.AddressPhysical2.Trim() == "")
                       this.AddressPhysical2 = customer.AddressPhysical2;

                   if (this.CityPhysical.Trim() == "")
                       this.CityPhysical = customer.CityPhysical;

                   if (this.StatePhysical.Trim() == "")
                       this.StatePhysical = customer.StatePhysical;

                   if (this.ZipPhysical.Trim() == "")
                       this.ZipPhysical = customer.ZipPhysical;

                   if (this.Licence.Trim() == "")
                       this.Licence = customer.Licence;
                   ...
                   if (this.HomePhone.Trim() == "" || this.HomePhone.Trim() == "(___)-___-____")
                       this.HomePhone = customer.HomePhone;

                   if (this.JobPhone.Trim() == "" || this.JobPhone.Trim() == "(___)-___-____")
                       this.JobPhone = customer.JobPhone;

                   if (this.Cellular.Trim() == "" || this.Cellular.Trim() == "(___)-___-____")
                       this.Cellular = customer.Cellular;
        ...
        private DataTable GetCustomerByFileNo(string FileNo)
        {
            DbRequestXmlCookRequestItem[] cookItems = new DbRequestXmlCookRequestItem[1];
            DbRequestXmlCooker.AttachCookItem("FileNo", SqlDbType.VarChar, 50, FileNo, ref cookItems);
            ...
            try
            {
                dt = exec.GetQuery("GetCustomerByFileNo", xmlDoc);

               return dt;
            }
            catch (Exception ex)
            {
                throw new Exception("Could not retrieve customer by file no.", ex);
            }
        }
		...
		{
			DbRequestXmlCookRequestItem[] cookItems = 
				new DbRequestXmlCookRequestItem[45];
			...
			DbRequestXmlCooker.AttachCookItem("Lastna1",
				SqlDbType.VarChar, 100, LastName1,
				ref cookItems);
			
			DbRequestXmlCooker.AttachCookItem("Lastna2",
				SqlDbType.VarChar, 100, LastName2,
				ref cookItems);
			...
			DbRequestXmlCooker.AttachCookItem("Adds1",
				SqlDbType.Char, 30, Address1,
				ref cookItems);
			
			DbRequestXmlCooker.AttachCookItem("Adds2",
				SqlDbType.Char, 30, Address2,
				ref cookItems);
			...
			DbRequestXmlCooker.AttachCookItem("SocSec",
				SqlDbType.VarChar, 100, SocialSecurity,
				ref cookItems);
			
			DbRequestXmlCooker.AttachCookItem("Licence",
				SqlDbType.VarChar, 20, Licence,
				ref cookItems);
            ...
            DbRequestXmlCooker.AttachCookItem("FileNo",
                SqlDbType.VarChar, 50, FileNo.ToString(),
                ref cookItems);
        ...
        //Cambio Pedro 3/31/2014 3:55 PM
		private static DataTable GetCustomerByCriteriaDB(string CustomerNo,string FirstName,string LastName1,string LastName2,string SocialSecurity,string Phone, bool IsBusiness, bool All, string EmployerSocialSecurity ,string BusinessName, int userId2, string LicenseNumber, string DOB)
		{
			DbRequestXmlCookRequestItem[] cookItems = 
			...
			DbRequestXmlCooker.AttachCookItem("BusinessName",
				SqlDbType.VarChar, 20, BusinessName,
				ref cookItems);

            //Cambio Pedro 3:53 PM 3/31/2014
            DbRequestXmlCooker.AttachCookItem("UserID",
              SqlDbType.Int, 0, userId2.ToString(),
              ref cookItems);

            DbRequestXmlCooker.AttachCookItem("LicenseNumber",
                SqlDbType.VarChar, 20, LicenseNumber,
                ref cookItems);

            DbRequestXmlCooker.AttachCookItem("DOB",
                SqlDbType.VarChar, 20, DOB,
                ref cookItems);
		...
		private XmlDocument GetInsertCustom_FXml()
		{
			...
			DbRequestXmlCooker.AttachCookItem("Adds1",
				SqlDbType.Char, 30, this.AddressPhysical1,
				ref cookItems);

            DbRequestXmlCooker.AttachCookItem("SameAsMailing",
            SqlDbType.Bit, 0, this.SamesAsMail.ToString(),
            ref cookItems);
			
			DbRequestXmlCooker.AttachCookItem("Adds2",
				SqlDbType.Char, 30, this.AddressPhysical2,
				ref cookItems);
			...
			DbRequestXmlCooker.AttachCookItem("State",
				SqlDbType.Char, 3, this.StatePhysical,
				ref cookItems);
		...
		private static Customer FillCustomerProperties(Customer customer)
		{
			...
			customer.Birthday   = (customer._dtCustomer.Rows[0]["BirthDay"]!=System.DBNull.Value)?((DateTime) customer._dtCustomer.Rows[0]["BirthDay"]).ToString("MM/dd/yyyy"):"";
            ...
            customer.FileNo = customer._dtCustomer.Rows[0]["FileNo"].ToString();
			...
			if (customer._dtCustomerAddr.Rows.Count != 0)
			{
                ...
                customer.SamesAsMail = (customer._dtCustomerAddr.Rows[0]["SameAsMailing"] != System.DBNull.Value) ? (bool)customer._dtCustomerAddr.Rows[0]["SameAsMailing"] : false;
			}
			return customer;
		}
		#endregion
			...
			if(mode == 2)
			{
				...
				switch (MutationTypeID)
				{
					case (int)MutationType.CUSTOMER:
                        ...
                        history.BuildNotesForHistory("File No.", oldCustomer.FileNo, this.FileNo, mode);
                        ...
                        history.BuildNotesForHistory("SameAsMailing", oldCustomer.SamesAsMail.ToString(), this.SamesAsMail.ToString(), mode);
